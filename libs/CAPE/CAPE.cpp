...
#ifdef OUTPUT_WITHOUT_REFINE
        Eigen::MatrixXi labels_matrix = Eigen::MatrixXi::Zero(depth_height, depth_width);
        int out_stacked_cell_id = 0;
        for (int row_id = 0; row_id < nr_vertical_cells; ++row_id){
                for (int col_id = 0; col_id < nr_horizontal_cells; ++col_id){
                    auto label = grid_plane_seg_map[row_id][col_id];
                    if (label != 0){
                        auto cell_row = out_stacked_cell_id / nr_horizontal_cells;
                        auto cell_col = out_stacked_cell_id % nr_horizontal_cells;
                        ...
                        for (auto i = label_row; i < label_row + cell_height; ++i) {
                            for (auto j = label_col; j < label_col + cell_width; ++j) {
                                labels_matrix.row(i)[j] = plane_merge_labels[label - 1];
                            }
                        }
                    }
                    ++out_stacked_cell_id;
...
//        std::ofstream of("output/labels_0.csv");
//        of << labels_matrix.reshaped<Eigen::RowMajor>(depth_height, depth_width)
